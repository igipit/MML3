...
namespace MML3
{
	...
	//===================================================================================================
	// Spline cubica nello spazio N dimensionale, parametrizzata con t \in [t_a, t_b]
	// Noti i  due punti estremi r_a ed r_b di una curva e le derivate tangenziali r'_a ed r'_b nei due punti
	// la spline cubica è un polinomio cubico rispetto al parametro t (in ciascuna delle N coordinate)
	//
	//      x(t)=cx[0] + cx[1] * t  + cx[2] * t^2 + cx[3] * t^3
	//      y(t)=cy[0] + cy[1] * t  + cy[2] * t^2 + cy[3] * t^3
	//      z(t)=cz[0] + cz[1] * t  + cz[2] * t^2 + cz[3] * t^3
	//
	// che passa per i due punti r_a ed r_b ed ha le stesse derivate tangenziali.
	// Posto r(t)=(x(t),y(t),z(t)), risulta
	// r(t_a)=r_a
	// d r/dt (t_a)= r'_a
	// r(t_b)=r_b
	// d r/dt (t_b)= r'_b
	//
	// Se nei due punti r_a ed r_b è nota solo la tangente alla curva (che è la derivata tangenziale normalizzata) si ha solo la garanzia
	// che la spline cubica avrà le stesse tangenti.
	...
	public:
		...
		// costruttore che assume il parametro t in [ta,tb]
		CubicSplineCurve(double ta, double tb, const double z[/*N*4*/]){ configure(ta,tb,z); }
		
		// costruttore che assume il parametro t in [0,1]
		CubicSplineCurve(const double z[/*N*4*/]){ configure(z); }


		//-------------------------------------------------------------------------------------------------------
		// Calcola  coefficienti della spline cubica r(t) :[0, 1] -> R^N, a partire dai valori di r=(r_x, r_y, ...)  e delle sue derivate negli estremi
		// In questa specializzazione il parametro è assunto nell'intervallo [0,1] ( t_a=0 e t_b=1)
		...
		// Come la precedente ma il parametro è nell'intervallo generico [t_a,t_b]
		...
		// e seconda della sline cubica  nel punto t specificato
		void	eval(double t, double value[N], double deriv[/*N*/], double deriv2[/*N*/])const;


		// ritornano i 4 coefficienti della spline cubica per ognuna delle coordinate :
		//      x=cx()[0] + cx()[1] * t  + cx()[2] * t^2 + cx()[3] * t^3
		//      y=cy()[0] + cy()[1] * t  + cy()[2] * t^2 + cy()[3] * t^3
		//      z=cz()[0] + cz()[1] * t  + cz()[2] * t^2 + cz()[3] * t^3
