...
namespace MML3
{
	...
	namespace GEO{
		...
		///=======================================================================================================
		// Curva chiusa e  convessa nel piano con l'origine (punto di coordinate 0,0) all'interno, formata da un 
		// insieme di splines cubiche che si raccordano con regolarità ai loro estremi. E' garantita la continuità 
		// complessiva e la continuità della derivata tangenziale. 
				
		// La curva è rappresentata mediante un insieme di N+1 punti della curva a cui sono associate altre informazioni geometriche
		// quali la  normale alla curva nel punto, lo jacobiano e la curvatura. 
		// I punti  sono rappresentati in coordinate polari (r,theta) con theta nel range [0, 2 pi). In virtù della convessità, 
		// ad ogni coppia di coordinate polari (r_i, theta_i),  con i=0,..N,  è associata una ed una sola  coppia di coordinate cartesiane 
		// P_i = r_i* (cos theta_i, sin theta_i). I punti sono internamente ordinati per angolo crescente. 
		// Tra due punti successivi P_i e P_i+1 la curva è rappresentata da una spline cubica che interpola i due punti 
		// ed ha (nei due punti)  la  normale esterna associata ai punti stessi. 
		...
		public:
			...
			// mappa che ad ogni angolo theta_i associa il punto Pi corredato di altre informazioni (normale, jacobiano, curvatura)
			typedef std::map<REAL, cpoint> point_map_t;
			typedef MML3::GEO::Vector<2, double> Vector2;
			typedef PointLocator<REAL> PointLocator;
			...
			///--------------------------------------------------------------------------------------------------------------------------
			// Definisce la curva 2D mediante la prescrizione di una poligonale chiusa convessa definita da un numero >=3  di punti (x,y). 
			// Si tratta di una curva chiusa di classe C1  costituita da rami cubici passanti per i punti medi dei 
			// segmenti della poligonale  e ivi tangenti. Se i punti non formano una poligonale convessa la funzione ritorna false
			// IN:
			//   x : coordinate x dei punti
			//   y : coordinate y dei punti
			// N.B. Non è necessario che i punti (x,y) forniti siano ordinati 
			bool		set_by_polygonal(const std::vector<REAL>& x, const std::vector<REAL>& y);

			///--------------------------------------------------------------------------------------------------------------------------
			// Versione specializzata della precedente per domini di plasticizzazione.
			//  Definisce il dominio 2D mediante la prescrizione di una poligonale chiusa convessa definita da un numero >=6  di punti (x,y). 
			//  La frontiera del dominio e' una curva chiusa di classe C1  costituita da rami cubici passanti per i punti medi dei 
			//  segmenti della poligonale  e ivi tangenti. 
			//  La poligonale deve possedere due coppie di punti speciali. Ognuna di queste coppie  definisce un segmento orizzontale che 
			//  attraversa l'asse y nella parte superiore ed inferiore della curva. In altre parole la poligonale deve essere "tagliata"
			//  orizzontalmente sopra e sotto. La lunghezza dei tratti orizzontali può essere piccola a piacere, l'importante è che
			//  intersechino l'asse verticale
			//  IN:
			//   x : coordinate x dei vertici della poligonale
			//   y : coordinate y dei vertici della poligonale
			//  RETURN:
			//   >0 : tutto ok
			//   <= 0 in caso si errore:
			...
			//      -3 : se i punti allineati orizzontalmente sono difettosi, ovvero se le ascisse non hanno segni opposti
			//			 o se le ordinate dei due segmenti non hanno segni opposti
			...
			// ritornano le ordinate delle intersezioni della curva con l'asse verticale
			...
			//-------------------------------------------------------------------------------------------------------------------------
			// Determina l'intersezione  del raggio alpha * (x,y) con la curva
			// IN
			//     (x,y)     : direzione del raggio
			//     lc        : puntatore ad un PointLocator, se non servisse basta passare NULLPTR
			//      tolerance: tolleranza relativa con cui l'algoritmo determina l'intersezione
			// OUT 
			//	  se lc!=NULLPTR in input, allora in output il PointLocator *lc conterra':
			//    P1,P2       : puntatori  ai  punti della curva   P1,P2 che sono gli estremi della spline che contiene l'intersezione alpha* (x,y) 
			...
			//--------------------------------------------------------------------------------------------------------------------------
			// Calcola il punto  di  intersezione P  tra la curva e la retta orizzontale di ordinata y nel quadrante definito dal segno di x
			...
			//--------------------------------------------------------------------------------------------------------------------------
			// Calcola entrambi i punti  di  intersezione  della retta orizzontale di ordinata y 
			// ritorna false se l'intersezione non esiste (la retta non interseca il dominio), true altrimenti
		...
		public:
			...
			// helper function che determina l'angolo di un punto (x,y) nel range [0,2pi)
			...
			// helper function che determina il raggio di un punto (x,y)
		...
		/**=======================================================================================================
		// Struttura  che individua un  punto P sulla curva mediante i due punti estremi P1 e P2 della spline che contiene 
		// il punto P e la coordinata t  della spline cubica. La coordinata t (pseudo angolo differenziale) che varia tra 0 ed 1
		// è solo in prima approssimazione uguale  a (P.theta - P1.theta())/Dtheta.
		// In generale sappiamo solo che t=0 quando theta = P1.theta e t=1 quando theta =P2theta
		*/
		template<typename REAL>
		struct PointLocator
		{
		...
		/**=======================================================================================================
		struct CircleMapped2DCurvePoint
		Classe che rappresenta un punto di una curva piana chiusa, convessa e di classe
		C1 di tipo class CircleMapped2DCurve, costituita da un insieme di splines
		cubiche.
		contiene e gestisce:
		1) coordinate polari del punto
		2) la  normale alla curva nel punto,
		3) la curvatura della curva nel punto
		4) lo Jacobiano della trasformazione di coordinate dalla coordinata adimensionale (0,1) della spline alla coordinata curvilinea
		*/
		...
		public:
			...
			// configura il punto attraverso le coordinate polari
			void set_polar_coord(double r, double theta)
			{
				r_ = r;
				theta_ = theta;
			}
			// configura il punto attraverso le coordinate cartesiane
			...
			// configura la normale direttamente 
			...
			// configura la normale attraverso la tangente
			void set_tangent(double tx, double ty)
			{
				set_normal(ty, -tx);
			}
			// configura lo jacobiano
			void	set_ds_dt(double t){ ds_dt_ = t; }
			// ritorna lo jacobiano
			double	ds_dt()const{ return ds_dt_; }
			// configura la curvatura
			...
			// vettore covariante rispetto alla coordinata [0,1]
